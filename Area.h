...
namespace mtm{
    ...
    protected:
		std::string name;
		MtmSet<std::string> reach_able;
        std::vector<GroupPointer> groups;
		class helpSub
		{
			string clan_to_compare;
		public:
			helpSub(string to_compare) : clan_to_compare(to_compare) {
			}
			~helpSub() = default;
			bool operator()(string current) const {
				return (current == clan_to_compare);
			}
		};
